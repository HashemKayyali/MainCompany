// Dev-only audit: reports PUBLIC-scope UI strings missing from the Arabic
// phrase map in src/lib/i18n.ts. Read-only — never modifies files.
// Companion to audit-admin-i18n.mjs (same extraction logic, public targets).
// Usage: node scripts/audit-public-i18n.mjs
...
// ── 2. Collect public-scope files (everything except admin) ──
const TARGET_DIRS = ['src/pages', 'src/components', 'src/contexts', 'src/config']
const EXCLUDE = [/[\\/]pages[\\/]admin[\\/]/, /[\\/]components[\\/]admin[\\/]/]
...
const files = TARGET_DIRS.flatMap(dir => walk(join(ROOT, dir))).filter(
  file => !EXCLUDE.some(re => re.test(file))
)

// ── 3. Extract candidate user-visible strings ──
const ATTR_RE =
  /(?:label|placeholder|title|hint|description|message|confirmLabel|cancelLabel|submitLabel|subtitle|heading|caption|cta|buttonLabel|emptyLabel|'aria-label')(?:=|:)\s*\{?\s*(['"])((?:(?!\1)[^\\]|\\.)+)\1/g
const JSX_TEXT_RE = />\s*([A-Z][^<>{}\n]*[a-zA-Z).!?%…])\s*</g
const TT_RE = /translateText\(\s*(['"])((?:(?!\1)[^\\]|\\.)+)\1\s*\)/g

const looksLikeCode = s =>
  /[<>{}\\]|className|https?:|^\d|^[a-z0-9-]+$|\.(tsx|ts|css|webp|svg|png|jpg)|^#|^\/|--|var\(|@/.test(s) ||
  !/[A-Za-z]{2}/.test(s)
...
const report = (phrase, file) => {
  ...
  // Skip Arabic-only literals (they are translations themselves, audited separately)
  if (/^[؀-ۿ]/.test(core)) return
...
console.log(`Public-scope files scanned: ${files.length}`)
