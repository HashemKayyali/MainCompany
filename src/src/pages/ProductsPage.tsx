...
export default function ProductsPage() {
  ...
  // ✅ show only products with showPrice !== false
  const visibleProducts = useMemo(() => {
    return products.filter(p => p.showPrice !== false)
  }, [products])
  ...
  const filtered = useMemo(() => {
    ...
    const byTags = visibleProducts.filter(p =>
      p.categoryTags?.some(t => t.toLowerCase() === filter.toLowerCase())
    )

    // merge + dedupe + enforce showPrice rule
    const merged = byCategory.concat(byTags).filter((p, i, arr) => arr.findIndex(x => x.slug === p.slug) === i)
    return merged.filter(p => p.showPrice !== false)
  }, [filter, categories, getProductsByCategory, visibleProducts])
    ...
    <section className="pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div
          ...
          className="mb-12"
        >
          {/* ✅ count reflects visible products */}
          <span className="section-label">// {visibleProducts.length} Products</span>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          ...
          {categories.map(c => {
            const count = getProductsByCategory(c.id).filter(p => p.showPrice !== false).length
            return (
