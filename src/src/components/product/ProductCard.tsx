      ...
      <div className={`group relative rounded-2xl overflow-hidden transition-all duration-700 hover:-translate-y-2 ${isDark ? 'bg-purple-500/[0.06] border border-purple-500/15 hover:border-prism-violet/40' : 'bg-white border border-violet-100/60 hover:border-violet-300 shadow-sm hover:shadow-xl hover:shadow-violet-100/40'}`}>
        ...
        <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-void-800 to-void-900">
          ...
          <div className="absolute top-3 left-3">
            {(() => {
              const bc = String(product.badgeColor || '')
              const isCss = bc.includes('linear-gradient')
              const clsBase =
                'inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider text-white shadow-lg max-w-[180px] overflow-hidden'
              return (
                <span
                  className={isCss ? clsBase : `${clsBase} bg-gradient-to-r ${bc}`}
                  style={isCss ? { backgroundImage: bc } : undefined}
                  title={product.badge}
                >
                  <span className="min-w-0 truncate">{product.badge}</span>
                </span>
              )
            })()}
          </div>
        ...
        <div className="relative p-5">
          ...
          {/* Price block (hidden when showPrice is false) */}
          {product.showPrice === false ? (
            <div className={`mt-4 py-3 border-t ${isDark ? 'border-purple-500/15' : 'border-violet-50'}`}>
              <span className={`text-[12px] font-semibold ${isDark ? 'text-purple-200/70' : 'text-gray-400'}`}>Price hidden</span>
            </div>
          ) : (
