...
export default function ContactActions({ form, validate }: { form: Record<string, string>; validate: () => boolean }) {
  ...
  const msg = () => { const p = products.find(x => x.slug === form.product); return ['Hello Eventies!','','Event request:','',`Name: ${form.name}`,`Email: ${form.email}`,form.phone ? `Phone: ${form.phone}` : '',`Service: ${p?.name || form.product}`,`Location: ${form.city}, ${form.country}`,form.address ? `Venue: ${form.address}` : '',form.message ? `\nNotes: ${form.message}` : ''].filter(Boolean).join('\n') }
  const wa = () => { if (!validate()) return; window.open(buildWhatsAppUrl(social.phone, msg()), '_blank') }
  const em = () => { if (!validate()) return; window.open(buildMailtoUrl(social.email, 'Event Request', msg()), '_blank') }
  return (
