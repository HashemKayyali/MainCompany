...
export default function AlbumCard({ album, onClick }: { album: GalleryAlbum; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className={`group text-left rounded-2xl overflow-hidden transition-all hover:-translate-y-1 w-full ${isDark ? 'bg-purple-500/[0.06] border border-purple-500/20 hover:border-prism-violet/30' : 'bg-white border border-gray-200 hover:border-violet-300 shadow-sm'}`}>
