...
export type HeaderThreeLinkItem = {
  title: string
  href: string
  icon: LucideIcon
  description?: string
...
export function HeaderThreeReference({
  scrolled = false,
  open = false,
  onToggleMenu,
  className,
  ...
  return (
    <header
      className={cn(
        ...
        scrolled && 'border-white/[0.08] bg-[rgba(7,9,22,0.86)] backdrop-blur-lg',
        className
      )}
    >
      <nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
        <div className="flex items-center gap-5">
          <a href="/" className="rounded-md p-2 transition-colors hover:bg-white/[0.08]">
            <WordmarkIcon className="h-4 text-white" />
          </a>
          <div className="hidden md:flex md:items-center md:gap-1">
            <a href="/" className="rounded-md px-4 py-2 text-sm font-medium text-white/78 transition-colors hover:bg-white/[0.08] hover:text-white">
              Home
            </a>
            ...
            <a href="/about" className="rounded-md px-4 py-2 text-sm font-medium text-white/78 transition-colors hover:bg-white/[0.08] hover:text-white">
              Company
            </a>
          </div>
        </div>
        ...
        <button
          ...
          className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-white/12 bg-white/[0.04] text-white md:hidden"
          aria-expanded={open}
...
export function HeaderThreeListItem({
  title,
  description,
  icon: Icon,
  className,
  href,
}: HeaderThreeLinkItem & { className?: string }) {
  return (
    <a
      href={href}
      className={cn(
        'flex w-full flex-row gap-x-3 rounded-md p-2 transition-colors hover:bg-white/[0.08]',
        className
      )}
    >
      <div className="flex aspect-square size-12 items-center justify-center rounded-md border border-white/[0.08] bg-white/[0.04] shadow-sm">
...
export const headerThreeProductLinks: HeaderThreeLinkItem[] = [
  ...
  { title: 'Integrations', href: '/contact', description: 'Plan multi-service event workflows', icon: LayersIcon },
  { title: 'API', href: '/about', description: 'Understand the marketplace structure', icon: CodeIcon },
]

export const headerThreeCompanyLinks: HeaderThreeLinkItem[] = [
  { title: 'About Us', href: '/about', description: 'Learn how Eventies works', icon: Users },
  { title: 'Customer Stories', href: '/customers', description: 'See who already trusts the platform', icon: Star },
  { title: 'Partnerships', href: '/contact', description: 'Talk to us about collaboration', icon: Handshake },
]

export const headerThreeCompanyLinks2: HeaderThreeLinkItem[] = [
