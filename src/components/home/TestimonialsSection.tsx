...
function TestimonialCard({ t, isDark }: { t: Testimonial; isDark: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`relative mx-auto max-w-[38rem] rounded-[20px] border p-4 sm:p-5 ${
        isDark
    ...
    >
      ...
      <div className="relative">
        {/* Quote icon */}
        <QuoteIcon
          className={`mb-3 h-7 w-7 ${isDark ? 'text-purple-500/30' : 'text-violet-200'}`}
        />

        {/* Quote text */}
        <blockquote className={`text-[0.92rem] font-medium leading-6 sm:text-[0.98rem] ${isDark ? 'text-purple-100/85' : 'text-gray-700'}`}>
          "{t.quote}"
        </blockquote>
...
export default function TestimonialsSection() {
  ...
  return (
    <section className="site-section relative" aria-label="Customer testimonials">
      {/* Background */}
      <div
                ...
                {testimonials.map((_, i) => (
                  <button
                    ...
                    className={`transition-all rounded-full ${
                      i === active
                        ? `w-5 h-2 ${isDark ? 'bg-purple-400' : 'bg-violet-500'}`
