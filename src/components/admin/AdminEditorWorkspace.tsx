    ...
    >
      {(title || hint) && (
        <div className="mb-2 flex items-start justify-between gap-3">
          <div>
            {title ? (
              <h3 className={cn('text-[13.5px] font-semibold', isDark ? 'text-white' : 'text-gray-900')}>{title}</h3>
            ) : null}
            {hint ? (
              <p className={cn('mt-1.5 text-[11px] leading-[1.35rem]', isDark ? 'text-purple-200/60' : 'text-gray-500')}>
                {hint}
              </p>
            ) : null}
          </div>
...
export default function AdminEditorWorkspace({
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      >
        <div className={cn('min-w-0 space-y-3.5', formPaneClassName)}>{children}</div>
        ...
        {preview ? (
          <aside className={cn('min-w-0 xl:sticky xl:top-2 xl:self-start', previewPaneClassName)}>
            ...
            >
              <div className="mb-2">
                <div className={cn('text-[9.5px] font-mono uppercase tracking-[0.16em]', isDark ? 'text-cyan-200/78' : 'text-violet-700/75')}>
                  {previewTitle}
                </div>
                {previewHint ? (
                  <p className={cn('mt-1.5 text-[11px] leading-[1.35rem]', isDark ? 'text-purple-200/58' : 'text-gray-500')}>
                    {previewHint}
                  </p>
                ) : null}
              </div>
      ...
      {footer ? (
        <div
          className={cn(
            'sticky bottom-0 z-10 rounded-[18px] border px-3.5 py-3 backdrop-blur-xl sm:px-4',
            isDark
