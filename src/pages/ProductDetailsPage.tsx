...
export default function ProductDetailsPage() {
  ...
  return (
    <section className="pt-32 pb-24"><div className="max-w-7xl mx-auto px-6">
      <Link to="/products" className={`inline-flex items-center gap-2 mb-8 text-[13px] font-mono tracking-wider uppercase ${isDark ? 'text-purple-300/80 hover:text-prism-violet' : 'text-gray-400 hover:text-violet-600'} transition-colors`}>{'<-'} Products</Link>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-14">
        {/* ── Left Column ── */}
        <div className="lg:col-span-3">
          <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, ease }}><ProductGallery images={product.gallery} name={product.name} /></motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1, ease }} className="mt-8">
            <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-gradient-to-r ${product.badgeColor} text-white mb-4`}>{product.badge}</span>
          ...
          {product.notes.length > 0 && <div className="mt-5 space-y-1.5">{product.notes.map((n, i) => <p key={i} className={`text-[13px] flex items-start gap-2 ${sub}`}><span className="text-prism-violet/80 mt-px">{'>'}</span>{n}</p>)}</div>}
          {(product.features.left.length > 0 || product.features.right.length > 0) && <div className="mt-7"><ProductFeatures features={product.features} /></div>}

          {/* ── Parts Section ── */}
          {parts.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25, ease }} className="mt-12">
                  ...
                  >
                    ...
                    {part.image ? (
                      <div className={`w-14 h-14 rounded-xl overflow-hidden shrink-0 ${isDark ? 'bg-purple-500/10' : 'bg-violet-50'}`}>
                        <img src={part.image} alt={part.name} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                      </div>
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`text-sm font-display font-bold whitespace-nowrap ${isDark ? 'text-cyan-400' : 'text-violet-600'}`}>{part.price} {part.currency}</span>
                      </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2, ease }}>
              <div className={`rounded-2xl overflow-hidden ${card}`}>
                <div className={`px-6 py-4 border-b ${isDark ? 'border-purple-500/20 bg-cyan-500/10' : 'border-violet-50 bg-cyan-50/50'}`}>
                  <span className={`text-[10px] font-mono uppercase tracking-[0.25em] ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>💬 Request a Quote</span>
                </div>
                <div className="p-6">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl text-[13px] font-medium flex items-center justify-between transition-all ${
                      isDark ? 'bg-purple-500/[0.07] hover:bg-purple-500/10 text-purple-100/90 border border-purple-500/15' : 'bg-violet-50/70 hover:bg-violet-50 text-gray-600 border border-violet-100'
                    }`}
                  >
                    <span>📋 What's included in the quote?</span>
                    ...
                    {quoteOpen && (
                      ...
                      >
                        <div className="pt-3 space-y-2">
                          {['Equipment rental & delivery', 'Professional on-site staff', 'Setup & dismantling', 'Custom branding (optional)', 'Insurance coverage', 'Spare parts availability'].map(item => (
                            <div key={item} className="flex items-center gap-2.5">
                              <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" className="text-cyan-400 shrink-0"><path d="M2 6l3 3 5-6" /></svg>
                  ...
                  <div className="space-y-2.5 mt-5">
                    <Link
                      ...
                      className="btn-primary w-full !rounded-xl !text-[13px] text-center block"
                    >
                      <span>📩 Get Your Quote</span>
                    </Link>
                    <a
                      href={`${social.whatsapp}?text=${encodeURIComponent(`Hi, I'd like a quote for "${product.name}" for my event.\n\nEvent details:\n- Date: \n- Location: \n- Expected guests: `)}`}
                      target="_blank"
            ...
            {parts.length > 0 && (
              <div className={`rounded-2xl p-5 ${card}`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-[10px] font-mono uppercase tracking-[0.25em] ${isDark ? 'text-purple-300/70' : 'text-gray-400'}`}>🔧 Spare Parts</span>
