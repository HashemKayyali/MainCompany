...
export default function AdminAdminsPage() {
  ...
  const handleAdd = () => {
    ...
    const ok = addAdmin(email, name, role)
    if (!ok) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className={`text-sm ${sub}`}>{admins.length} administrators</p>
        </div>
      ...
      {/* ✅ تنبيه لو الداتا مش راجعة فيها role أو admins فاضية */}
      {admins.length === 0 && (
        <div
          className={`mb-6 px-4 py-3 rounded-xl text-sm ${
            isDark
        ...
        >
          No admins loaded. This usually means AuthContext is filtering admins or the database query doesn’t return role.
        </div>
      )}
            ...
            <div key={a.id} className="card-surface rounded-2xl p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {/* ✅ إذا role أصلاً مش موجود */}
                  {(a as any).role == null && (
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded-md font-mono ${
                        isDark ? 'bg-red-400/15 text-red-300' : 'bg-red-50 text-red-600'
                      }`}
                    >
                      role missing
                    </span>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Admin">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input className="form-field" type="email" value={email} onChange={e => setEmail(e.target.value)} />
          </div>
          <div>
            <label className={`block text-[12px] mb-1.5 font-medium ${sub}`}>Role</label>
            <select className="form-field" value={role} onChange={e => setRole(e.target.value as any)}>
          ...
          <div className="flex gap-3 justify-end mt-6">
            ...
            <button onClick={handleAdd} className="btn-primary !px-6 !py-2.5 !rounded-xl !text-xs">
              Add Admin
            </button>
          </div>
        </div>
