...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    { icon: '📸', label: 'Gallery', value: galleryAlbums.length, to: '/admin/gallery', gradient: 'from-fuchsia-500 to-pink-500', shadow: 'shadow-fuchsia-500/20' },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-8">{stats.map(s => (
      ...
      <div className={`rounded-2xl p-6 ${isDark ? 'bg-purple-500/[0.07] border border-purple-500/20' : 'bg-white border border-gray-200 shadow-sm'}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[{ label: '+ Add Product', to: '/admin/products', icon: '🚲' },{ label: '+ Add Part', to: '/admin/parts', icon: '🔧' },{ label: '+ Add Customer', to: '/admin/customers', icon: '👥' },{ label: '+ Add Category', to: '/admin/categories', icon: '📂' },{ label: '+ Add Album', to: '/admin/gallery', icon: '📸' }].map(a => (
