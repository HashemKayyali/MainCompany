...
export default function AdminProductsPage() {
  ...
  const normalizeHex = (hex: string | undefined, fallback: string) => {
    ...
    if (/^#[0-9a-fA-F]{3}$/.test(h)) { const r = h[1], g = h[2], b = h[3]; return `#${r}${r}${g}${g}${b}${b}`.toLowerCase() }
  ...
  const tabCls = (t: TabKey) =>
    `px-4 py-2 rounded-xl text-[12px] font-semibold transition-all border ${
      tab === t
        ? isDark ? 'bg-prism-violet/15 text-prism-violet border-prism-violet/40' : 'bg-violet-50 text-violet-700 border-violet-200'
        : isDark ? 'bg-transparent text-purple-200/70 border-transparent hover:bg-purple-500/[0.08]' : 'bg-transparent text-gray-500 border-transparent hover:bg-gray-50'
    }`

  const openNew = () => {
    setEdit(null)
    const g = parseGradient(EMPTY.badgeColor)
    setForm({ ...EMPTY, categoryTags: '', notes: '', featuresLeft: '', featuresRight: '', gallery: [], badgeFromHex: g.fromHex, badgeToHex: g.toHex, showPrice: true })
    setTab('basic')
    setModal(true)
  }
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      return p.name.toLowerCase().includes(needle) || p.slug.toLowerCase().includes(needle) || (p.badge || '').toLowerCase().includes(needle) || catName.toLowerCase().includes(needle)
    })
  ...
  const moveGalleryImage = (idx: number, dir: -1 | 1) =>
    setForm((f: any) => {
      const g = [...(f.gallery || [])]; const ni = idx + dir
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3">
        <div><h1 className={`font-display text-2xl font-bold ${txt}`}>Products</h1></div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${soft}`}>
            ...
            <input value={q} onChange={e => setQ(e.target.value)} className={`bg-transparent outline-none text-[12px] w-[240px] sm:w-[320px] ${isDark ? 'placeholder:text-purple-200/40' : 'placeholder:text-gray-400'}`} placeholder="Search by name, badge, category…" />
            <span className={`text-[10px] font-mono px-2 py-0.5 rounded-lg border ${isDark ? 'border-purple-500/20 text-purple-200/60' : 'border-gray-200 text-gray-500'}`}>{filtered.length}/{products.length}</span>
          </div>
          <button onClick={openNew} className="btn-primary !px-4 !py-2 !text-xs !rounded-xl">+ Add Product</button>
        </div>
      </div>
      ...
      <div className={`rounded-2xl overflow-hidden ${soft}`}>
              ...
              {filtered.map(p => (
                <tr key={p.slug} className={`border-b ${divider} transition-colors ${rowHover}`}>
                  <td className="px-5 py-3.5">
                    <button type="button" onClick={() => openEdit(p)} className="w-full text-left">
                      <div className="flex items-center gap-3">
                        {p.heroImage ? (
                          <img src={p.heroImage} alt="" className="w-12 h-9 rounded-xl object-cover border border-white/5" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                        ) : (
                          <div className={`w-12 h-9 rounded-xl ${soft2} flex items-center justify-center text-[10px] ${sub}`}>No Img</div>
                        )}
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className={`text-sm font-semibold ${txt} truncate`}>{p.name}</span>
                            {p.featured && <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${isDark ? 'border-prism-violet/30 bg-prism-violet/15 text-prism-violet' : 'border-violet-200 bg-violet-50 text-violet-700'}`}>FEATURED</span>}
                          </div>
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell">
                    {p.badge ? (
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-bold text-white max-w-[140px] truncate ${String(p.badgeColor || '').includes('linear-gradient') ? '' : `bg-gradient-to-r ${p.badgeColor}`}`} style={String(p.badgeColor || '').includes('linear-gradient') ? { backgroundImage: p.badgeColor } : undefined}>{p.badge}</span>
                  ...
                  <td className={`px-5 py-3.5 text-xs font-mono ${sub}`}>
                    <span className={`px-2 py-1 rounded-full border ${isDark ? 'border-purple-500/20 bg-purple-500/[0.05]' : 'border-gray-200 bg-gray-50'}`}>{p.gallery.length} 📷</span>
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <button onClick={() => openEdit(p)} className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold border transition ${isDark ? 'border-purple-500/20 bg-purple-500/[0.06] text-purple-200/90' : 'border-gray-200 bg-white text-gray-700'}`}>Edit</button>
                      <button onClick={() => setConfirm(p.slug)} className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold border transition ${isDark ? 'border-red-400/20 bg-red-400/10 text-red-200/90' : 'border-red-200 bg-red-50 text-red-700'}`}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {filtered.length === 0 && (
          <div className="p-10 text-center">
            ...
            <button onClick={openNew} className="btn-primary !mt-5 !px-5 !py-2.5 !text-xs !rounded-xl">+ Add Product</button>
          </div>
        )}
      </div>
        ...
        <div className="space-y-5">
            ...
            {tab === 'basic' && (
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${soft2}`}>
                        ...
                        <input type="color" value={form.badgeFromHex || DEFAULT_FROM} onChange={e => setBadgeGradient(e.target.value, form.badgeToHex || DEFAULT_TO)} className="w-8 h-7 p-0 bg-transparent border-0 cursor-pointer rounded" />
                      </div>
                      <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${soft2}`}>
                        <span className={`text-[11px] ${sub}`}>To</span>
                        <input type="color" value={form.badgeToHex || DEFAULT_TO} onChange={e => setBadgeGradient(form.badgeFromHex || DEFAULT_FROM, e.target.value)} className="w-8 h-7 p-0 bg-transparent border-0 cursor-pointer rounded" />
                      </div>
                      {/* Preview */}
                      <div className={`px-3 py-2 rounded-xl border flex-1 min-w-0 ${isDark ? 'border-purple-500/20' : 'border-gray-200'}`}>
                        <span className={`inline-flex px-3 py-1 rounded-full text-[11px] font-bold text-white truncate ${String(form.badgeColor || '').includes('linear-gradient') ? '' : `bg-gradient-to-r ${form.badgeColor}`}`} style={String(form.badgeColor || '').includes('linear-gradient') ? { backgroundImage: form.badgeColor } : undefined}>
                ...
                <div>
                  ...
                  <input value={typeof form.categoryTags === 'string' ? form.categoryTags : (form.categoryTags || []).join(', ')} onChange={e => setForm({ ...form, categoryTags: e.target.value })} className="form-field" placeholder="Interactive, Wellness" />
                </div>
            ...
            {tab === 'content' && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea value={typeof form.notes === 'string' ? form.notes : (form.notes || []).join('\n')} onChange={e => setForm({ ...form, notes: e.target.value })} className="form-field resize-none" rows={5} placeholder={'Requires fresh ingredients\nNeeds 1x power outlet\nSetup time: ~30 min'} />
                </div>
              </div>
            )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  ...
                  {(form.gallery || []).map((url: string, idx: number) => (
                    <div key={idx} className="relative group">
                      ...
                      {idx === 0 && (
                        <span className={`absolute top-1.5 left-1.5 text-[8px] font-bold px-1.5 py-0.5 rounded-md ${isDark ? 'bg-cyan-400/25 text-cyan-200' : 'bg-violet-100 text-violet-700'}`}>HERO</span>
                      )}
                      <div className={`absolute inset-0 rounded-xl flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity ${isDark ? 'bg-black/60' : 'bg-white/70'}`}>
                        {idx > 0 && (
                          <button type="button" onClick={() => moveGalleryImage(idx, -1)} className={`text-[10px] px-2 py-1 rounded-lg ${isDark ? 'bg-purple-500/30 text-white' : 'bg-violet-100 text-violet-700'}`}>◀</button>
                        )}
                        {idx < (form.gallery || []).length - 1 && (
                          <button type="button" onClick={() => moveGalleryImage(idx, 1)} className={`text-[10px] px-2 py-1 rounded-lg ${isDark ? 'bg-purple-500/30 text-white' : 'bg-violet-100 text-violet-700'}`}>▶</button>
                        )}
                        <button type="button" onClick={() => removeGalleryImage(idx)} className="text-[10px] px-2 py-1 rounded-lg bg-red-500/40 text-white">✕</button>
                      </div>
                    </div>
                  ))}

                  {/* ONE Add Image button */}
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={addOption} className={`text-[11px] font-semibold px-3 py-1 rounded-lg border ${isDark ? 'border-prism-violet/30 bg-prism-violet/15 text-prism-violet' : 'border-violet-200 bg-violet-50 text-violet-700'}`}>+ Add Option</button>
                  </div>
                  <div className="space-y-2">
                    {(form.quickOptions || []).map((opt: any, idx: number) => (
                      <div key={idx} className={`flex flex-col sm:flex-row sm:items-center gap-2 p-3 rounded-xl ${soft2}`}>
                        ...
                        <button onClick={() => removeOption(idx)} className={`px-3 py-2 rounded-lg text-[11px] font-semibold border ${isDark ? 'border-red-400/20 bg-red-400/10 text-red-200/90' : 'border-red-200 bg-red-50 text-red-700'}`}>Remove</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <div className="sm:ml-auto text-[11px] leading-4">
              <p className={sub}>Hero image: <span className={txt}>{(form.gallery || [])[0] ? 'Gallery #1' : 'None'}</span></p>
