...
import { extractProductOrderMeta, injectProductOrderMeta } from '../lib/product-order'

/**
 * FOUND-017 — products service, ported from src/services/products.service.ts
 * with CLIENT INJECTION (factory closes over the injected SupabaseClient so
 * every function keeps its original signature; both the browser client and
 * the per-request server client work). The dbToApp/appToDb mappings are
 * VERBATIM — they are the behavioral spec.
 */
...
function dbToApp(row: ProductRow): Product {
  ...
  return {
    ...
    gallery: row.gallery ?? [],
    quickOptions: (row.quick_options as unknown as Product['quickOptions']) ?? [],
    notes,
...
function appToDb(p: Product): ProductInsert {
  return {
    ...
    gallery: p.gallery || [],
    quick_options: (p.quickOptions as unknown as ProductInsert['quick_options']) ?? [],
...
export function createProductsService(supabase: SupabaseClient<Database>) {
  ...
  async function update(slug: string, product: Partial<Product>): Promise<Product> {
    ...
    if (product.categoryId !== undefined)
      dbData.category_id = product.categoryId ? product.categoryId : null
    ...
    if (product.quickOptions !== undefined)
      dbData.quick_options = product.quickOptions as unknown as ProductUpdate['quick_options']
    ...
    if (product.minimumRentalDays !== undefined)
      dbData.minimum_rental_days = Number(product.minimumRentalDays)
    if (product.bufferBeforeDays !== undefined)
      dbData.buffer_before_days = Number(product.bufferBeforeDays)
    if (product.bufferAfterDays !== undefined)
      dbData.buffer_after_days = Number(product.bufferAfterDays)
