...
| I18N-015 | P1/P2 | i18n | Arabic typography check: line-height/heading scale on 6 templates | NEW | L | — | I18N-005 | snapshots approved | RTL-V | TODO:AR typography pass — CAT-024 visual pass |
...
| CACHE-003 | P2 | cache | Invalidation integration tests per entity (edit→tag→fresh) | NEW | H | D | FOUND-021, CACHE-001 | ADM-INV green | ADM-INV | DONE:revalidate helper + auth-gated /api/revalidate (401/403 verified); per-entity admin-mutation E2E is P6 |
...
| CAT-002 | P2 | catalog | Home hero (WebGL) as ssr:false island; fallback static hero | REARCHITECT | M | P | CAT-001 | no SSR crash; LCP budget | PERF-SL | DONE:WebGL hero client island (ssr:false shader) + static gradient hero + LCP img |
...
| CAT-004 | P2 | catalog | Home gallery strip (server data, lazy images) | REARCHITECT | M | P | DATA-006 | budgets | PERF | DONE:home sections (categories + popular + marketing stack) |
...
| CAT-010 | P2 | catalog | Deleted/inactive product → notFound() | NEW | M | E | CAT-007 | HTTP 404 | SEO-404 | DONE:real HTTP 404 (ADR-23) — verified EN+AR |
...
| CAT-019 | P2 | catalog | Search dialog rebuild over server lists (client island, bounded) | REARCHITECT | M | P | DATA-001 | results parity | E | DONE:search dialog island over cached server index |
...
| SEO-014 | P2 | seo | Group A parity run + approval report | NEW | H | E | SEO-013, CAT-020 | QG-P2 SEO section pass | SEO-PAR | DONE:EN parity 0 unexplained deltas after full visual port (local) |
...
| ADMIN-003 | P6 | admin | Admin UX integration of destructive-op enforcement (typed confirm, recent-auth re-prompt flows — UX ONLY, never the enforcement) + final end-to-end enforcement verification | NEW | H | S | ADMIN-001, SEC-016 | ADM-RA green + BYPASS suite green end-to-end | ADM-RA, BYPASS | BLOCKED_BY_OWNER_STAGING:rollout-disabled UI/handler integration+ADM-RA+BYPASS code contracts green; live personas deferred |
...
| ADMIN-006 | P6 | admin | Product/category forms + AR fields (I18N-020) + uploads via Edge Fn | REFACTOR | H | D | ADMIN-020, ADMIN-021, DBMIG-004 | CRUD green; AR saved | E | BLOCKED_BY_OWNER_STAGING:EN/AR validated mutation/upload contracts ready; live CRUD/upload persistence deferred |
...
| ADMIN-012 | P6 | admin | Interior port: Admins page (role ops via trusted boundary) | REFACTOR | M | S | ADMIN-003 | BYPASS green for role ops | ADM-RA, BYPASS | BLOCKED_BY_OWNER_STAGING:trusted role/removal RPC boundary+final-superadmin invariant green; live personas deferred |
...
| ADMIN-014 | P6 | admin | Revoked-admin mid-session E2E (layout AND RPC deny) | NEW | H | S | ADMIN-001 | green | ADM-GATE | BLOCKED_BY_OWNER_STAGING:execution-time role/profile checks+fixture persona green; live revocation deferred |
...
| ADMIN-016 | P6 | admin | Edge Fn quota (SEC-013, provisional 30/h) | HARDEN | M | S | ADMIN-015 | 31st sign denied + event | I | BLOCKED_BY_OWNER_STAGING:atomic hour/day quota RPC+denial audit authored; live 31st-sign evidence deferred |
...
| ADMIN-019 | P6 | admin | Group E cutover readiness report | NEW | H | S | ADMIN-* | QG-P6 pass | — | BLOCKED_BY_OWNER_STAGING:CODE_SIDE_COMPLETE; QG-P6 awaits DBMIG-010 application and live evidence |
...
| SEC-012 | cross | security | (=ADMIN-015 spec) upload-preset constraint enforcement — preset definition + signer diff + UPL-NEG negative test | HARDEN | M | S | — | see ADMIN-015 | I | BLOCKED_BY_OWNER_STAGING:signed preset/folder/MIME/size/delete contracts green; live UPL-NEG deferred |
| SEC-013 | cross | security | (=ADMIN-016 spec) signing quota storage + thresholds config | HARDEN | M | S | — | see ADMIN-016 | I | BLOCKED_BY_OWNER_STAGING:durable atomic quota storage/RPC+audit authored; live calibration deferred |
...
| DBMIG-010 | P6 | dbmig | Wave E: AAL2/recent-auth DB enforcement migrations per APPROVED SEC-018 design (RPC assurance params / RLS checks / EXECUTE revocations) — staging verified; ⛔ prod | NEW | H | S,D | DBMIG-002, SEC-018 | BYPASS suite green on staging; ⛔ approved | BYPASS | BLOCKED_BY_OWNER_STAGING:migration authored after approval+static BYPASS contracts green; application/live probes deferred; direct DELETE revocation absent until Group E |
| DBMIG-011 | P6 | dbmig | Wave E prod verification + bypass re-probe on prod | NEW | H | S | DBMIG-010 | BYPASS green on prod | BYPASS | BLOCKED:DBMIG-010 approved/authored but staging application and QG-P6 evidence are absent; production untouched |
...
| A11Y-007 | P2 | a11y | Reduced-motion: prefers-reduced-motion honored by reveal/scroll/hero animations (port usePerfMode intent) | NEW | M | P | CAT-021 | media-query E2E | E | DONE:prefers-reduced-motion honored (hero + Reveal) |
...
| ADMIN-026 | P6 | admin | Interior port: Notifications send page (broadcast preview preserved) | REFACTOR | M | D | ADMIN-011 | broadcast E2E | E | BLOCKED_BY_OWNER_STAGING:idempotent superadmin broadcast handler/RPC ready; live audience evidence deferred |
| ADMIN-027 | P6 | admin | Interior port: Users page | REFACTOR | M | S | ADMIN-003 | parity | E | BLOCKED_BY_OWNER_STAGING:authorized searchable user grid and safe contracts ready; live parity/mutations deferred |
...
| ID | Phase | Domain | Title | Type | Risk | Flags | Deps | Acceptance | Tests | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| SEC-018 | P6 | security | Destructive-operation trusted-boundary DESIGN/SPECIFICATION: per 05-matrix row choose mechanism (A RPC self-enforce via auth.jwt() assurance inspection / B RLS / C EXECUTE-revoke + handler), define exact RPC param & claim checks, migration list for DBMIG-010, BYPASS test plan — ⛔ human design approval | NEW | H | S,D | — | approved design doc; every matrix row has a chosen mechanism + test | design review | DONE:APPROVED_BY_OWNER_SECURITY_DECISION 2026-07-13; mandatory conditions incorporated |
