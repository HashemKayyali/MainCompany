...
export default function ContactActions({ form, validate }: { form: Record<string, string>; validate: () => boolean }) {
  ...
  const msg = () => { const p = products.find(x => x.slug === form.product); return ['Hello Bike Land!','','Event booking:','',`Name: ${form.name}`,`Email: ${form.email}`,form.phone ? `Phone: ${form.phone}` : '',`Product: ${p?.name || form.product}`,`Location: ${form.city}, ${form.country}`,form.address ? `Venue: ${form.address}` : '',form.message ? `\nNotes: ${form.message}` : ''].filter(Boolean).join('\n') }
